...
class UserFrame(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        Label(self, text='[room count , age , area , owner, user ').grid(row=5, column=1, sticky='SW')
        ...
        for i in range(ad_number):
            Label(self, text=str(reader[i][:5])).grid(row=i + 7, column=1, sticky='SW')
...
class NewAdsFrame(Frame):
    def __init__(self, parent, controller):
        ...
        for i in range(len(new_properties)):
            Label(self, text=str(reader[new_properties[i]])).grid(row=i + 2, column=0, sticky='SW')
...
class AdFrame(Frame):
    def __init__(self, parent, controller):
        ...
        Label(self, text="owner").grid(row=3, column=0)
        Entry(self, textvariable=self.owner).grid(row=3, column=1)

        Button(self, borderwidth=4, text="Add", width=10, pady=4, command=self.new_ad).grid(row=5, column=1)
        Button(self, borderwidth=4, text="Return", width=10, pady=4,
               command=lambda: self.controller.show_frame("UserFrame")).grid(row=6, column=1)

    def new_ad(self):
        try:
            db.newad(self.room_count.get(), self.age.get(), self.area.get(), self.owner.get(),
